...
fn format_level2_neural_prompt(adapted: &str) -> String {
    [
        ...
        "When JSON is requested, escape line breaks inside JSON string values as \\n; never place literal line breaks inside quoted JSON strings.",
        "When JSON is requested, escape embedded double quotes and backslashes according to JSON syntax.",
        "When JSON is requested, never place raw tabs, carriage returns, or other control characters inside JSON string values.",
