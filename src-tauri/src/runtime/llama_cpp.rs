...
fn model_spec_for_workload(
    ...
    let priority = capability_priority(
    ...
    model_spec(priority[0]).ok_or_else(|| {
        format!("model_spec_missing:{}", priority[0])
    })
}

fn next_retry_budget(
    current: u32,
    allowed: u32,
) -> Option<u32> {
    if current >= allowed {
...
impl LlamaCppHttpExecutor {
    pub fn new(
        ...
        let client = Client::builder()
            ...
            .build()
            .map_err(|e| {
                format!("llama_http_client_failed:{e}")
            })?;
    ...
    fn execute_attempt(
        ...
        let raw = response
            .text()
            .map_err(|e| {
                format!("llama_response_read_failed:{e}")
            })?;

        let attempt_wall_time_ms =
    ...
    pub fn execute_one(
        ...
        let compiled =
            compile_certification_prompt(workload)?;

        let spec =
            model_spec_for_workload(workload)?;
        ...
        let allowed_max_tokens =
            spec.allowed_max_tokens.max(1);

        let mut max_tokens =
            generation
                .max_tokens
                .min(allowed_max_tokens)
                .max(1);
        ...
        let tokens_per_second =
            if attempts > 1 && wall_time_ms > 0 {
                attempt.output_tokens as f64
                    / (
                        wall_time_ms as f64
                        / 1000.0
                    )
    ...
    for LlamaCppHttpExecutor
{
    fn execute(
        ...
        if concurrency == 0 {
            return Err(
                "concurrency_must_be_at_least_one"
                    .into(),
            );
        }
        ...
        for chunk in
            workloads.chunks(concurrency as usize)
        {
            let mut results =
                std::thread::scope(|scope| {
                    ...
                    for handle in handles {
                        match handle.join() {
                            ...
                            Err(_) => {
                                return Err(
                                    "llama_parallel_worker_panicked"
                                        .into(),
                                );
                            }
                        }
...
mod tests {
    ...
    fn retry_budget_expands_but_never_exceeds_ceiling() {
        assert_eq!(
            next_retry_budget(256, 512),
            Some(512)
        );

        assert_eq!(
            next_retry_budget(384, 1024),
            Some(768)
        );

        assert_eq!(
            next_retry_budget(1024, 2048),
            Some(2048)
        );

        assert_eq!(
            next_retry_budget(512, 512),
            None
        );
    }
}
