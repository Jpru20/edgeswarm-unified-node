...
fn run() -> Result<(), String> {
    ...
    let authenticated_email =
        user.email.as_deref().unwrap_or("").trim().to_lowercase();
    ...
    let expires_at = aal2.expires_at
    ...
    match select_wallet_row(&rows, &hardware.hardware_id)? {
        ...
        WalletRowDecision::ClaimLegacy { .. } => {
            return Err(
                "legacy_wallet_claim_requires_separate_review".into()
            );
        }
        ...
        WalletRowDecision::CreateDevice => {
            ...
            let exact = exact.ok_or_else(|| {
                format!("wallet_insert_not_visible_after_http_{status}")
            })?;
            ...
            let recovered =
                DeviceWallet::from_private_key(&recovered_key)?;
